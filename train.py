...
class ESRGANDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, hr_dir, lr_dir, scale=4, patch_size=128, is_train=True):
        ...
        logger.info(f"找到 {len(self.hr_images)} 对{'训练' if is_train else '验证'}图像")
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            # 验证模式：使用整个图像或中心裁剪
            if hr_h > 800 or hr_w > 800:  # 验证图像太大时进行中心裁剪
                # 中心裁剪
                crop_size = min(hr_h, hr_w, 800)
                start_h = (hr_h - crop_size) // 2
                start_w = (hr_w - crop_size) // 2
                
                hr_img = hr_img[start_h:start_h+crop_size, start_w:start_w+crop_size]
                
                lr_start_h = start_h // self.scale
                lr_start_w = start_w // self.scale
                lr_crop_size = crop_size // self.scale
                lr_img = lr_img[lr_start_h:lr_start_h+lr_crop_size, lr_start_w:lr_start_w+lr_crop_size]
...
class ESRGANTrainer:
    ...
    def setup_dataloader(self):
        ...
        logger.info(f"训练批次大小: {self.config['training']['batch_size']}")
        
        # 验证数据集
        if 'val_hr_dir' in self.config['datasets'] and 'val_lr_dir' in self.config['datasets']:
            val_dataset = ESRGANDataset(
                hr_dir=self.config['datasets']['val_hr_dir'],
                lr_dir=self.config['datasets']['val_lr_dir'],
                scale=self.config['scale'],
                patch_size=self.config['datasets']['patch_size'],
                is_train=False
            )
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for lr_imgs, hr_imgs in tqdm(self.val_dataloader, desc="验证"):
                lr_imgs = lr_imgs.to(self.device)
                hr_imgs = hr_imgs.to(self.device)
        ...
        logger.info(f"验证PSNR: {avg_psnr:.2f} dB")
    ...
    def train(self):
        ...
        for epoch in range(self.config['training']['epochs']):
            ...
            # 验证
            if self.val_dataloader is not None and 'val_freq' in self.config['training'] and (epoch + 1) % self.config['training']['val_freq'] == 0:
                ...
                if avg_psnr > self.best_psnr:
                    ...
                    logger.info(f"新的最佳PSNR: {avg_psnr:.2f} dB")
        ...
        if self.val_dataloader is not None:
            final_psnr = self.validate()
            logger.info(f"最终PSNR: {final_psnr:.2f} dB, 最佳PSNR: {self.best_psnr:.2f} dB")
